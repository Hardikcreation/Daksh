...
export default function PartnerSupportPage() {
  ...
  // Fetch partner profile, orders, and tickets on mount
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const token = localStorage.getItem("partnerToken");
        const res = await axios.get(`${BASE_URL}/api/partners/profile`, {
    ...
    const fetchTickets = async () => {
      try {
        const token = localStorage.getItem("partnerToken");
        const res = await axios.get(`${BASE_URL}/api/tickets/partner`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setTickets(res.data);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 mb-6 text-center">Partner Support</h1>
